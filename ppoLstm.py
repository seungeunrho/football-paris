...
#         cat = torch.cat([player_embed, ball_embed, left_team_embed,  right_team_embed], 2)
#         hidden = state_dict["hidden"]
#         out, lstm_hidden = self.lstm(cat, hidden)
...
#         return prob, v, lstm_hidden
    ...
    def make_batch(self, data):
        ...
        a,r,done_mask, prob_a = torch.tensor(a_batch, device=self.device).permute(1,0,2), torch.tensor(r_batch, device=self.device).permute(1,0,2), \
                                torch.tensor(done_batch, dtype=torch.float, device=self.device).permute(1,0,2), \
                                torch.tensor(prob_a_batch, device=self.device).permute(1,0,2)
